...
/**
 * Title: CSC130HansrajD.Program5
 * Filename: MinePanel.java
 * Date Written: December 12, 2017
 * Due Date: December 16, 2017
 * Description: Create a MinePanel object that applies all of the traditional rules of the minesweeper game.
 * The use of recursion to expose safe cells helps simplifies the code required.
 * 
...
class MinePanel extends JPanel implements MouseListener
{
	...
	public void removeAdjacentButtons(Graphics g,int r, int c) {
		...
		if(r > 15 || r < 0 || c > 15 || c < 0 || button[r][c].isMined() || button[r][c].isShowing())
			return;
		else if(button[r][c].getNoOfMinesAround() > 0)
			button[r][c].showIt(g);
		else {
			button[r][c].showIt(g);
