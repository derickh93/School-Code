...
public class CircularArrayQueue<E> implements QueueADT<E>{
	 ...
	 */
	public synchronized void enqueue(E newItem) throws QueueFullException
	{	
	 ...
	 */
	public synchronized void enqueueFront(E newItem)
	{
		if(count == contents.length)
			this.expandCapacity();
		for(int i = 0; i < count; i++)
	 ...
	 */
	public synchronized E dequeueRear()
	{
		if (isEmpty())
	...
	public synchronized E rear() throws QueueEmptyException {
		if (isEmpty())
			throw new QueueEmptyException("CircularArrayQueue");
		else
