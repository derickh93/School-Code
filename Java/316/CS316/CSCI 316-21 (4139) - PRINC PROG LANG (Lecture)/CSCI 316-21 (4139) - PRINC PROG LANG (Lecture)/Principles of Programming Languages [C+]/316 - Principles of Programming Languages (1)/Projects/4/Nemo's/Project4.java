...
public abstract class Project4 extends LexAnalyzer
{
	...
	// <statement> --> "{" <SList> "}" | "while" "(" <expr> ")" <statement> | 
	//					"if" "(" <expr> ")" <statement> [ "else" <statement> ] | <assignment>

	{
		...
		else if ( state == State.While )
		{
			...
			if ( state == State.LParen )
			{
				...
				{
					displayln("		if_f goto " + ++loopNum);
					locLoopNum = loopNum;
					getToken();
					statement();
					displayln("		goto " + (locLoopNum - 1));
					displayln(locLoopNum + ":");
				}
				else
					errorMsg(1);
			}
		...
		else if ( state == State.If )
		{
			...
			if ( state == State.LParen )
			{
				...
				if ( state == State.RParen )
				{
					...
					if ( state == State.Else )
					{
						displayln("		goto " + ++loopNum);
						displayln(locLoopNum + ":");
						getToken();
						statement();
						displayln((locLoopNum + 1) + ":");
					}
					else
						displayln(locLoopNum + ":");
				}
	...
	{
		statement();
		while ( state == State.LBrace || state == State.Id || state == State.While || 
					state == State.If || state == State.Else )
			statement();
	}
