...
public class AddE extends E{
	...
	Type TypeEval(HashMap<String, String> state) {
		...
		if(termEval != null){
			...
			if(eEval != null){
				if(termEval.keyword.equals("int") && eEval.keyword.equals("int")){
					return termEval.IntClone();
				}
				else if(termEval.keyword.equals("int") && eEval.keyword.equals("float")){
					return termEval.FloatClone();
				}
				else if(termEval.keyword.equals("float") && eEval.keyword.equals("int")){
					return termEval.FloatClone();
				}
				else if(termEval.keyword.equals("float") && eEval.keyword.equals("float")){
					return termEval.FloatClone();
				}else{
					return null;
				}
			}else{
				System.out.println("+ only wroks on int and float");
				return null;
			}
		}
		return null;
