...
public class Equal extends BoolPrimary {

	String sign;
	E e_, e__;
	
	Equal(String s, E e, E e1){
		sign = s;
		e_ = e;
		e__ = e1;
	}
	...
	Type TypeEval(HashMap<String, String> state) {
		...
		if(e_Eval != null){
			...
			if(e__Eval != null){
				...
				else if((e_Eval.keyword.equals("boolean")) && (e__Eval.keyword.equals("boolean"))){
					return e__Eval.BooleanClone();
				}else{
					System.out.println("== cannot be applied to ("+e_Eval.keyword+", "+e__Eval.keyword+")");
					return null;
				}
			}else{
				return null;
