 ...
 * Description: Defines the methods that will be used to create the easy mode for a basic memory game.
 * In the easy mode there is a delay of 2000 mili secs before wrong selections are flipped back over.
 * There are methods to save and restore the instance of the application. There is a method to
 * reset the game and there is also an onClick method to determine what happens upon each click. There are also
 * methods to determine what happens once the user clicks on the menu options.
 * Once the first level has been completed the second(higher) level will be started.
 *
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
     ...
     * game will move on to the higher level of play.
     *
    ...
    public void onClick(View view) {
        ...
        if(matches == 10) {
            Toast.makeText(getApplicationContext(), R.string.strComplete, Toast.LENGTH_SHORT).show();
            for (int i = 0; i < 20; i++) {
                buttonArr[i].setClickable(false);
            }
     ...
     * indicating that the game has been reset. Then each button will be set reset and clickable again.
     * The arraylist of images will be shuffled and then count and the amount of matches will also be
     * reset to 0.
     */
    public void reset() {
        Collections.shuffle(array_image);
        clickCount = 0;
        matches = 0;
        Toast.makeText(getApplicationContext(),R.string.strGameReset,Toast.LENGTH_SHORT).show();
        for (int i = 0; i < 20; i++) {
            buttonArr[i].setClickable(true);
            buttonArr[i].setImageResource(R.drawable.gameimagedefault);
        }
    }
